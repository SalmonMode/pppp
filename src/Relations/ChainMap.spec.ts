...
describe("ChainMap", function () {
  describe("One Unit", function () {
    ...
    before(function () {
      ...
      chainMap = new ChainMap(new StrainMap([unit]));
    });
  ...
  describe("One Unit (Get Chain of Unrelated Unit)", function () {
    ...
    it("should throw NoSuchChainError", function () {
      expect(() => chainMap.getChainOfUnit(unrelatedUnit)).to.throw(
        NoSuchChainError
      );
    });
  });
  describe("Two Units", function () {
    ...
    before(function () {
      ...
      chainMap = new ChainMap(new StrainMap([unitA, unitB]));
    });
    it("should have one chain", function () {
      expect(chainMap.chains.length).to.equal(1);
    });
    it("should get chain of unitA", function () {
      const expectedChain = chainMap.chains[0];
      assertIsObject(expectedChain);
      expect(chainMap.getChainOfUnit(unitA)).to.equal(expectedChain);
    });
    ...
    it("should provide only unit B as the available when no isolated units are passed", function () {
      expect(chainMap.getHeadsWithoutUnits([])).to.deep.equal([unitB]);
    });
  });
  describe("Chain Building", function () {
    ...
    describe("Different Density (More Density First)", function () {
      ...
      before(function () {
        ...
        chainMap = new ChainMap(
          new StrainMap([unitA, unitB, unitC, unitD, unitE])
        );
      });
      ...
      it("should have one chain with just A, C, and B", function () {
        const aChain = chainMap.getChainOfUnit(unitA);
        const bChain = chainMap.getChainOfUnit(unitB);
        const cChain = chainMap.getChainOfUnit(unitC);
      ...
      it("should have one chain with just D", function () {
        const dChain = chainMap.getChainOfUnit(unitD);
        expect(dChain.units.length).to.equal(1);
      });
      it("should have one chain with just E", function () {
        const eChain = chainMap.getChainOfUnit(unitE);
    ...
    describe("Same Density, Different Presence (More Presence First)", function () {
      ...
      before(function () {
        ...
        chainMap = new ChainMap(new StrainMap([unitA, unitB, unitC, unitD]));
      });
    ...
    describe("Complex Interconnections", function () {
      ...
      before(function () {
        ...
        aChain = chainMap.getChainOfUnit(unitA);
        bChain = chainMap.getChainOfUnit(unitB);
        cChain = chainMap.getChainOfUnit(unitC);
        dChain = chainMap.getChainOfUnit(unitD);
        eChain = chainMap.getChainOfUnit(unitE);
        fChain = chainMap.getChainOfUnit(unitF);
        gChain = chainMap.getChainOfUnit(unitG);
        hChain = chainMap.getChainOfUnit(unitH);
        iChain = chainMap.getChainOfUnit(unitI);
      });
      ...
      it("should have 4 connections between chains for A-B-C and D-E-F", function () {
        expect(
          chainMap.getNumberOfConnectionsBetweenChains(aChain, dChain)
        ).to.equal(4);
      });
      it("should have 4 connections between chains for D-E-F and A-B-C", function () {
        ...
        expect(
          chainMap.getNumberOfConnectionsBetweenChains(dChain, aChain)
        ).to.equal(4);
      });
      it("should have 0 connections between chains for A-B-C and G-H-I", function () {
        expect(
          chainMap.getNumberOfConnectionsBetweenChains(aChain, gChain)
        ).to.equal(0);
      });
      it("should have 0 connections between chains for G-H-I and A-B-C", function () {
        // same as the other one, but looking it up from the other direction to make sure that's working as well
        expect(
          chainMap.getNumberOfConnectionsBetweenChains(gChain, aChain)
        ).to.equal(0);
      });
      it("should have 4 connections between chains for D-E-F and G-H-I", function () {
        expect(
          chainMap.getNumberOfConnectionsBetweenChains(dChain, gChain)
        ).to.equal(4);
      });
      it("should have 4 connections between chains for G-H-I and D-E-F", function () {
        // same as the other one, but looking it up from the other direction to make sure that's working as well
        expect(
          chainMap.getNumberOfConnectionsBetweenChains(gChain, dChain)
        ).to.equal(4);
      });
      it("should recognize when it doesn't own an unrelated chain", function () {
        ...
        expect(chainMap.ownsChain(unrelatedChain)).to.be.false;
      });
      it("should throw NoSuchChainError when trying to get strength map for unrelated chain", function () {
        const unrelatedChain = new IsolatedDependencyChain([
          new TaskUnit([], new Date(), new Date()),
        ]);
        expect(() => chainMap.getStrengthMapForChain(unrelatedChain)).to.throw(
          NoSuchChainError
        );
      });
      it("should throw NoSuchChainError when trying to get number of connections between unrelated and owned chains", function () {
        ...
        expect(() =>
          chainMap.getNumberOfConnectionsBetweenChains(unrelatedChain, aChain)
      ...
      it("should throw NoSuchChainError when trying to get number of connections between owned and unrelated chains", function () {
        ...
        expect(() =>
          chainMap.getNumberOfConnectionsBetweenChains(aChain, unrelatedChain)
        ).to.throw(NoSuchChainError);
      });
    });
